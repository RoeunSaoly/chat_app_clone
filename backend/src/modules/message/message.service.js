...
export const sendMessage = async (senderId, conversationId, content, messageType = "text") => {
  ...
  // 4. Create message_reads entries for delivered status
  const memberIds = members.map((m) => m.user_id);
  await messageModel.createMessageReads(message.id, memberIds);
...
export const handleTyping = async (conversationId, userId, isTyping) => {
  await messageModel.updateTypingStatus(conversationId, userId, isTyping);

  const typingUsers = await messageModel.getTypingUsers(conversationId, userId);

  emitToConversation(conversationId, isTyping ? "typing" : "stop_typing", {
    conversation_id: conversationId,
    user_id: userId,
...
export const getMessages = async (conversationId, limit = 50, offset = 0) => {
  ...
  const messagesWithReads = await Promise.all(
    messages.map(async (msg) => {
      const reads = await messageModel.getMessageReads(msg.id);
      return {
        ...msg,
        read_by: reads,
      };
